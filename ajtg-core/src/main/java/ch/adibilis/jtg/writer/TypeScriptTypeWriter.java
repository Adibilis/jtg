...
public class TypeScriptTypeWriter {
    ...
    public List<TypeScriptFile> generateTypes(Map<String, Type> namedTypes, GeneratorConfig config) {
        return generateTypes(namedTypes, config, Map.of());
    }

    /**
     * Generates plain TypeScript files for {@code namedTypes} and resolves imports
     * against both the generated files (default-export) and {@code externalNamedFiles}
     * (named-export; typically files emitted by another writer such as ZodTypeWriter).
     * <p>
     * Without {@code externalNamedFiles}, a plain DTO that references a Zod-emitted
     * enum or schema produces an unresolved identifier — the reference appears in the
     * body but no import is registered.
     */
    public List<TypeScriptFile> generateTypes(Map<String, Type> namedTypes, GeneratorConfig config, Map<String, TypeScriptFile> externalNamedFiles) {
        ...
        for (TypeScriptFile file : files) {
            resolveImports(file, fileMap, simpleNameToType, externalNamedFiles, config);
    ...
    private void resolveImports(TypeScriptFile file, Map<String, TypeScriptFile> fileMap,
                                 Map<String, Type> simpleNameToType,
                                 Map<String, TypeScriptFile> externalNamedFiles,
                                 GeneratorConfig config) {
        String fileName = extractTypeName(file.getRelativePath());
        Type fileType = simpleNameToType.get(fileName);
        if (fileType == null) return;

        // Default-export targets generated by this writer
        ...
        // Named-export targets emitted by another writer (e.g. ZodTypeWriter's enums
        // and validated-object schemas). Same body-reference check, but registered as
        // named imports since those files have no default export.
        for (Map.Entry<String, TypeScriptFile> entry : externalNamedFiles.entrySet()) {
            String typeName = entry.getKey();
            TypeScriptFile targetFile = entry.getValue();
            if (fileMap.containsKey(typeName)) continue; // local file wins
